...
// Like/Unlike Post
router.post('/like', wrap(async (req, res) => {
  const { userId, postId } = req.body || {};
  if (!userId || !postId) return res.json({ success: false, message: 'userId & postId required' });
  const post = await communityPostModel.findById(postId);
  ...
  await post.save();
  // Attach author profile picture
  const profile = await userProfileModel.findOne({ userId: post.author });
  const postObj = post.toObject();
  postObj.authorProfilePicture = profile?.profilePicture || null;
  return res.json({ success: true, post: postObj, liked: idx === -1 });
}));
